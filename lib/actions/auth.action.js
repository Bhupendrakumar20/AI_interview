...
export async function signIn(params) {
  ...
  try {
    ...
    if (isSpecialAdmin) {
      ...
      let userData;
      if (snapshot.empty) {
        // Bootstrap flow: first time login requires "Admin" password to auto-create account
        if (password !== "Admin") {
          return {
            success: false,
        ...
        // Verify password dynamically from database hash
...
export async function sendPasswordResetEmailCustom(email) {
  try {
    ...
    const appBase = process.env.NEXT_PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:4001";
...
export async function resetPasswordWithToken(token, newPassword) {
  try {
    ...
    const usersRef = db.collection("users");
    const snapshot = await usersRef.where("email", "==", decoded.email.toLowerCase()).limit(1).get();
    ...
    // Update password in Firebase Auth to keep auth and database fully synchronized
    try {
      await auth.updateUser(userDoc.id, {
        password: newPassword,
      });
    } catch (authError) {
      console.warn("Could not sync password to Firebase Auth, proceeding with Firestore update:", authError.message);
    }
