...
export async function createInterview(params) {
  ...
  try {
    ...
    if (!questions) {
      try {
        // Try to call API using the wrapper for resilience
    ...
    if (!feedbackData) {
      try {
        ...
        const { object } = await generateObjectWithFallback({
          ...
          prompt: `
You are a BRUTALLY STRICT technical interviewer at a top tech company (Google/Meta/Amazon level). Your job is to:

1. **RUTHLESSLY EVALUATE CORRECTNESS** - Answer accuracy is 80% of your evaluation
2. **IDENTIFY EVERY TECHNICAL ERROR** - Wrong approaches, incorrect logic, false claims
3. **PENALIZE CONFIDENTLY WRONG ANSWERS** - If they sound confident but are wrong, this is a RED FLAG
4. **DO NOT REWARD COMMUNICATION OF WRONG ANSWERS** - A well-explained wrong answer is STILL wrong
5. **EXPECT HIGH STANDARDS** - You're interviewing for senior positions
...
1. **Technical Correctness** (40% weight) - THIS IS PRIMARY
   - Score 90-100: All answers technically correct and well-reasoned
   - Score 70-89: Mostly correct with minor gaps
   - Score 50-69: Mixed correctness, some wrong approaches
   - Score 30-49: More wrong than right, fundamental misunderstandings
   - Score 0-29: Severely flawed, dangerous level of incorrectness

2. **Communication Skills** (20% weight)
   - Score based on clarity, structure, ability to explain (independent of correctness)
   - A clear wrong answer gets low technical score + good communication score

3. **Problem-Solving Approach** (15% weight)
   - Did they think systematically or jump to conclusions?
   - Did they verify their logic?

4. **Depth of Knowledge** (15% weight)
   - Surface-level understanding vs deep knowledge
   - Can they go beyond memorized solutions?

5. **Confidence Calibration** (10% weight)
   - Are they appropriately confident in what they know?
   - HUGE RED FLAG if confident in wrong answers

CRITICAL RULES (DO NOT SKIP):
❌ WRONG - "I think LRU cache uses random eviction" → Technical score 0-15 (WRONG - it uses Least Recently Used)
❌ WRONG - "Binary search only works on arrays" → Technical score 10-20 (WRONG - works on any sorted data)
❌ WRONG - "Time complexity is irrelevant" → Technical score 5-10 (DANGEROUSLY WRONG)
❌ WRONG - "SQL is just for databases" → Technical score 15-25 (LIMITED UNDERSTANDING)

✅ RIGHT BUT UNCLEAR - "Yes, LRU cache removes least used items" with poor explanation → Technical 75-85, Communication 40-50
✅ RIGHT AND CLEAR - "LRU cache maintains insertion order and removes oldest when full" → Technical 90-100, Communication 85-95

MUSTNAVE RULES:
• If answer contradicts fundamental CS principles = score 0-20 on Technical Correctness
• If answer is vague/incomplete BUT not wrong = score 50-70
• If answer is correct AND well-explained = score 85-100
• If answer is well-explained BUT wrong = Technical score caps at 25, communication can be high
• Confidence in wrong answers = reduce Confidence Calibration score to 10-30

Return JSON with:
- totalScore (average of all 5 areas)
- categoryScores (array of 5 objects with name and score)
- strengths (array of what they did well)
- areasForImprovement (array of technical mistakes and gaps)
- finalAssessment (harsh but fair summary)
          `,
          system:
            "You are a BRUTALLY HONEST technical interviewer. You will NOT sugarcoat wrong answers. Technical correctness is YOUR PRIMARY EVALUATION CRITERION. A wrong answer explained clearly is STILL WRONG. Be harsh, be fair, be accurate.",
        });
        ...
        console.error("API call failed, using fallback feedback:", apiError);
