...
async function fetchFantasticJobs() {
  try {
    ...
    const response = await fetch('https://fantastic.jobs/api', {
      method: 'GET',
      headers: {
...
export async function fetchJobs(searchQuery = "developer jobs", options = {}) {
  ...
  try {
    ...
    // Only attempt JSearch if the API key is configured
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.data && Array.isArray(data.data)) {
            transformedJobs = transformJobsData(data.data);
          }
        }
      } catch (err) {
    ...
    // Always attempt to fetch from Fantastic Jobs API
    const fantasticJobs = await fetchFantasticJobs();

    // Merge job listings from both sources
    const allJobs = [...transformedJobs, ...fantasticJobs];
