...
async function fetchFantasticJobs() {
  try {
    ...
    if (!contentType || !contentType.includes("application/json")) {
      throw new Error(`Fantastic Jobs API returned non-JSON content-type: ${contentType}`);
    }
    ...
    console.error("Error fetching from Fantastic Jobs API:", error);
    return [];
  }
}
...
export async function fetchJobs(searchQuery = "developer jobs", options = {}) {
  ...
  try {
    ...
    if (allJobs.length > 0) {
      console.log(`Successfully fetched ${allJobs.length} total jobs (JSearch: ${transformedJobs.length}, Fantastic: ${fantasticJobs.length})`);
      return {
        success: true,
        jobs: allJobs,
        source: "api",
