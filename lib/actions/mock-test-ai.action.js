...
export async function evaluateMockTest({ question, code, language, transcript, whiteboardSummary, company, role }) {
  try {
    ...
    const { object } = await generateObject({
      // NOTE: "gemini-3.5-flash" is not a valid Gemini model name and will
      // always fail. Using "gemini-2.0-flash" to match the working model
      // used elsewhere in this file. Swap to "gemini-1.5-flash" or
      // "gemini-2.5-flash" if you'd prefer those instead.
      model: google("gemini-2.0-flash", { apiKey: GEMINI_API_KEY }),
