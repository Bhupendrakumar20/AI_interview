...
function getKeysForProvider(provider) {
  ...
  if (provider === 'google') {
    return uniqueKeys.filter(k => k.startsWith('AIzaSy'));
  }
  return uniqueKeys;
}
...
/**
 * Custom groq model provider wrapper.
 * Dynamically fails over and rotates keys, supporting fallback between Groq and Google.
 */
export function groq(modelName = "llama-3.3-70b-versatile", options = {}) {
  // Return a lazy/virtual language model that intercepts doGenerate and doStream to handle rotation & fallback
  return {
    specificationVersion: 'v2',
    provider: 'failover-provider',
    ...
    async doGenerate(generateOptions) {
      let lastError;
      // Prefer groq first, fall back to google
      const providersToTry = ['groq', 'google'];
      ...
      for (const provider of providersToTry) {
        ...
        for (const key of keys) {
          ...
          try {
            let actualModel;
            if (provider === 'groq') {
              const groqInstance = createGroq({ apiKey: key, structuredOutputs: false });
              actualModel = wrapLanguageModel(groqInstance(modelName || "llama-3.3-70b-versatile"));
            } else {
              actualModel = googleProvider("gemini-3.6-flash", { ...options, apiKey: key });
            }
            return await actualModel.doGenerate(generateOptions);
          } catch (error) {
            lastError = error;
            ...
            if (msg.includes("429") || msg.includes("TooManyRequests") || msg.includes("quota") || msg.includes("Rate limit")) {
              ...
              console.warn(`⚠️ [AI-Provider] groq wrapper transient error with key ${key.substring(0, 10)}...: ${msg}`);
            }
          }
        }
      }
      throw lastError || new Error(`All providers and keys exhausted in groq() failover`);
    },
        ...
        for (const key of keys) {
          ...
          try {
            ...
            if (msg.includes("429") || msg.includes("TooManyRequests") || msg.includes("quota") || msg.includes("Rate limit")) {
              ...
              console.warn(`⚠️ [AI-Provider] groq wrapper transient error during streaming with key ${key.substring(0, 10)}...: ${msg}`);
            }
          }
        }
      }
      throw lastError || new Error(`All providers and keys exhausted in groq() stream failover`);
    }
  };
}
          ...
          for (const key of keys) {
            ...
            try {
              if (provider === 'groq') {
                ...
                const url = `https://generativelanguage.googleapis.com/v1beta/models/${(model === "gemini-pro" || model === "gemini-2.0-flash") ? "gemini-3.6-flash" : (model || "gemini-3.6-flash")}:generateContent?key=${key}`;
