  ...
  const speak = useCallback(
    async (text) => {
      try {
        ...
        console.warn("Cloud TTS failed, falling back to Browser Web Speech API:", error.message);
        try {
          speakWebSpeech(text);
        } catch (fallbackError) {
          const errorMessage = fallbackError.message || "Failed to play speech";
          setState((prev) => ({
            ...prev,
            isLoading: false,
            error: errorMessage,
