// local-runner.cjs — CommonJS wrapper for dsa-socket-server-prod.js
// The main local-runner.js was converted to ESM; this CJS version is for
// the standalone Node socket server that uses require().

const { spawnSync } = require('child_process');
const fs = require('fs');
const path = require('path');

function executeLocally(sourceCode, language, stdin = '') {
  ...
  try {
    ...
    if (runResult.error) {
      if (runResult.error.code === 'ETIMEDOUT') {
        return { success: false, output: '', error: 'Time Limit Exceeded (5 seconds)', exitCode: -1 };
      }
      return { success: false, output: '', error: `Runtime Error: ${runResult.error.message}`, exitCode: -1 };
    }
    ...
    cleanup();
    return { success: false, output: '', error: `Local Runner Exception: ${err.message}`, exitCode: -1 };
  }

  function cleanup() {
    try {
      if (runFile && fs.existsSync(runFile)) fs.unlinkSync(runFile);
      if (exeFile && fs.existsSync(exeFile)) fs.unlinkSync(exeFile);
      if (lang === 'java') {
        const runDir = path.dirname(runFile);
        if (fs.existsSync(runDir)) {
          const files = fs.readdirSync(runDir);
          for (const file of files) fs.unlinkSync(path.join(runDir, file));
          fs.rmdirSync(runDir);
        }
      }
    } catch (e) {
