...
function initializeGenAI() {
  const apiKey = process.env.GOOGLE_GENERATIVE_AI_API_KEY || process.env.GROQ_API_KEY;
  if (!apiKey) {
    throw new Error(
      "Neither GOOGLE_GENERATIVE_AI_API_KEY nor GROQ_API_KEY is set in environment variables"
    );
  }
...
export async function generateSpeech(text, config = {}) {
  try {
    ...
    }, "textToSpeechGeneration", "tts-service", { prompt: text });
