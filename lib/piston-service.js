...
let resolvedPistonBaseUrl = null;

async function getPistonBaseUrl() {
  if (resolvedPistonBaseUrl) return resolvedPistonBaseUrl;

  const configuredUrl = process.env.PISTON_API_URL || 'http://127.0.0.1:2000/api/v2/piston';

  try {
    const res = await fetch(`${configuredUrl}/runtimes`, { signal: AbortSignal.timeout(2000) });
    if (res.ok) {
      resolvedPistonBaseUrl = configuredUrl;
      return resolvedPistonBaseUrl;
    }
  } catch (e) {
    // ignore
  }

  if (configuredUrl.endsWith('/piston')) {
    const strippedUrl = configuredUrl.replace(/\/piston$/, '');
    try {
      const res = await fetch(`${strippedUrl}/runtimes`, { signal: AbortSignal.timeout(2000) });
      if (res.ok) {
        resolvedPistonBaseUrl = strippedUrl;
        console.log(`[Piston] Auto-detected Docker Piston base URL: ${resolvedPistonBaseUrl}`);
        return resolvedPistonBaseUrl;
      }
    } catch (e) {
      // ignore
  ...
  // Fallback to public Piston API if local is unreachable (e.g. on Vercel)
  try {
    const publicUrl = 'https://emkc.org/api/v2/piston';
    const res = await fetch(`${publicUrl}/runtimes`, { signal: AbortSignal.timeout(3000) });
    if (res.ok) {
      resolvedPistonBaseUrl = publicUrl;
      console.log(`[Piston] Falling back to public Piston API: ${resolvedPistonBaseUrl}`);
      return resolvedPistonBaseUrl;
    }
  } catch (e) {
    // ignore
...
const EXECUTION_TIMEOUT_MS = 20_000; // 20 s — generous for cold-start runtimes and compilation
...
export const executeCode = async ({ sourceCode, language, stdin = '', version = '*' }) => {
  const PISTON_BASE_URL = await getPistonBaseUrl();
