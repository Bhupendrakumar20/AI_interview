...
class PerUserRateLimiterManager {
  ...
  getAllStats() {
    ...
    let activeUsers = 0;
    for (const userMap of this.limiters.values()) {
      activeUsers += userMap.size;
    }
    return {
      model: spec.name,
      activeUsers,
      globalRequestsPerMinute: spec.rpm,
      perUserRequestsPerMinute: Math.max(1, Math.floor(spec.rpm / this.maxConcurrentUsers)),
    };
  }
