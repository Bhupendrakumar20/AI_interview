 ...
 */
export function checkActionRateLimit(userId, action) {
  let allowed = false;
  let remaining = 0;

  switch (action) {
    case "gemini":
      allowed = checkGeminiRateLimit(userId);
      remaining = 500; // Approximate
      break;
    case "room-create":
      allowed = checkRoomCreationRateLimit(userId);
      remaining = 10;
      break;
    default:
      allowed = true;
