  ...
  try {
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (updateData.score !== undefined) {
        const statsRef = db.collection("interview_buddy_stats").doc(currentUser.uid);
...
export async function recordVoteWithTransaction(sessionId, votedUserId, voteType, currentUser) {
  try {
    const result = await db.runTransaction(async (transaction) => {
      ...
      const voteRef = db.collection("session_votes").doc(voteId);
      ...
      const userStatsRef = db.collection("user_stats").doc(votedUserId);
