...
export async function updateSessionWithTransaction(sessionId, updateData, currentUser) {
  try {
    // Sessions are stored in the top-level `interviews` collection.
    // Use a direct document reference instead of a collectionGroup query which
    // requires a full document path when filtering by documentId().
    const sessionRef = db.collection("interviews").doc(sessionId);
    const sessionDocCheck = await sessionRef.get();
    if (!sessionDocCheck.exists) {
...
export async function recordVoteWithTransaction(sessionId, votedUserId, voteType, currentUser) {
  try {
    // Use direct top-level collection document ref for the session
    const sessionRef = db.collection("interviews").doc(sessionId);
    const sessionDocCheck = await sessionRef.get();
    if (!sessionDocCheck.exists) {
      throw new Error("Session not found");
