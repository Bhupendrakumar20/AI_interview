...
export async function updateSessionWithTransaction(sessionId, updateData, currentUser) {
  try {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      // Verify creator or participant status
      const isCreator = sessionData.createdBy === currentUser.uid;
      const isParticipant = sessionData.participants?.includes(currentUser.uid);
      if (!isCreator && !isParticipant) {
        throw new Error("Unauthorized: User is not the creator or participant of this session");
      }
