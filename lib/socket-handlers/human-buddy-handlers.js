 ...
 */
export function initializeHumanBuddyHandlers(io) {
  ...
  buddyNamespace.on('connection', (socket) => {
    ...
    socket.on('join_session', async (data) => {
      try {
        ...
        console.log(`[join_session] ${username} (${userId}) joining session ${sessionCode}`);
        ...
        socket.join(`buddy_${sessionId}`);
        
        console.log(`✓ ${username} joined socket room: buddy_${sessionId}`);
        ...
        console.log(`[Buddy] Emitted session_joined to ${username}. Room: buddy_${sessionId}`);

        // Notify OTHER participants in the room that new user joined
        // ✅ FIX 4: Include full remote user object
        socket.to(`buddy_${sessionId}`).emit('user_joined_session', {
          userId,
          username,
          user: {
            userId,
        ...
        console.log(`[Buddy] Broadcasting user_joined_session to room buddy_${sessionId}`);
        console.log(`✓ ${username} joined session ${sessionId} with role: ${role}`);
      } catch (error) {
...
export { BUDDY_SESSION_STATUS, BUDDY_ROLES };
